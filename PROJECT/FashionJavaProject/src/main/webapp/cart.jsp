						...
						<form method="post" action="CustomerController">
							<table>
								...
								<tbody>
									<%
									/* SELECT * FROM book 
									INNER JOIN subproduct ON `subproduct`.`sproductid`=book.`sproductid`
									WHERE customerid = 1 */

									Connection cc = new DBUtil().getConnectionData();
									String qry = "SELECT subproduct.`simage`,subproduct.`sproductname`,subproduct.`price`,book.`qty`,book.`customerid`,book.`sproductid`,book.bookid FROM book INNER JOIN subproduct ON `subproduct`.`sproductid`=book.`sproductid` WHERE customerid = ?";
									...
									%>
									<tr>
										<td class="cart__price"><h5><%=rs.getString(7)%></h5></td>
										<td class="product__cart__item">
											<div class="product__cart__item__pic">
												<img src="productimage\<%=rs.getString(1)%>" alt=""
										...
										<td class="cart__price"><h5><%=rs.getString(3)%></h5></td>
										...
										<td><button type="submit" value="delete" name="action" class="btn btn-danger">
                        					<input type="hidden" name="sproductid" value="<%= rs.getInt(6) %>">
										...
										<input type="hidden" name="sproductid" value="<%= rs.getInt(6) %>">
                   							<input type="hidden" name="price" value="<%= rs.getInt(3) %>">								
										<button type="submit" value="checkout" name="action" class="btn btn-success">
                                       Checkout
                                    </button></td>
									</tr>
									<%
