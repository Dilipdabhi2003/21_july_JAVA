...
import com.dao.AssessmentVerifyDao;
import com.model.AssessmentUserModel;


@WebServlet("/AssessmentVerifyController")
public class AssessmentVerifyController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
   
    public AssessmentVerifyController() {
        super();
...
String action=request.getParameter("action");
		...
		if(action.equalsIgnoreCase("submit"))
		{
			...
			AssessmentVerifyDao dao=new AssessmentVerifyDao();
			...
			else
			{
				...
				final String password = "hdceoyydfkacuajv";
		...
		else if(action.equalsIgnoreCase("confirm"))
		{
			...
			AssessmentUserModel f=new AssessmentUserModel();
			HttpSession session=request.getSession(false);
			f=(AssessmentUserModel)session.getAttribute("UserData");
			f.setPassword(pswd);
			f.setId(f.getId());
			AssessmentVerifyDao dao=new AssessmentVerifyDao();
