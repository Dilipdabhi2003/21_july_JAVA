...
import com.dao.Qus9VerifyDao;
import com.model.Qus9Model;



/**
 * Servlet implementation class Qus9VerifyController
 */
@WebServlet("/Qus9VerifyController")
public class Qus9VerifyController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(action.equalsIgnoreCase("submit"))
		{
			...
			Qus9VerifyDao dao=new Qus9VerifyDao();
			f=dao.getVerify(email);
			
			if(f==null)
			{
				request.setAttribute("invalidemail", "Email Address Not Valid");
				request.getRequestDispatcher("change9.jsp").forward(request, response);
			}
			else
			{
				String emailid=f.getEmail();
				System.out.println(emailid);
				String username=f.getFname()+" "+f.getLname();
		...
		else if(action.equalsIgnoreCase("Verify"))
		{
			...
			if(G_otp.equalsIgnoreCase(E_otp))
			{
				...
				request.getRequestDispatcher("chage.jsp").forward(request, response);
			}
			else
			{
		...
		else if(action.equalsIgnoreCase("confirm"))
		{
			...
			Qus9Model f=new Qus9Model();
			HttpSession session=request.getSession(false);
			f=(Qus9Model)session.getAttribute("UserData");
			f.setPassword(pswd);
			f.setUid(f.getUid());
			Qus9VerifyDao dao=new Qus9VerifyDao();
			...
			if(r>0)
			{
				response.sendRedirect("change9.jsp");
